...
class PureMF(BasicModel):
    ...
    def bpr_loss(self, users, pos, neg, unique_user=None, pos_item_index=None, pos_item_mask=None):
        ...
        # 组合pos item的特征
        keep_pos_emb = self.embedding_item(keep_pos_items.long())
        pos_emb = torch.cat([keep_pos_emb, replaceable_items_feature])
        # 使用传统的bpr
...
class LightGCN(BasicModel):
    ...
    def getEmbedding(self, users, keep_pos_items, need_replace_pos_item, replaceable_items_feature, neg_items):
        all_users, all_items = self.computer()
        users_emb = all_users[users]
        pos_emb = all_items[keep_pos_items]
        neg_emb = all_items[neg_items]
        # 拼接需要替换的item
        pos_emb = torch.cat([pos_emb, replaceable_items_feature])
        # 换取替换了之后的item列表
        pos_items = torch.cat([keep_pos_items, need_replace_pos_item])
    ...
    def bpr_loss(self, users, pos, neg, unique_user, pos_item_index=None, pos_item_mask=None):
        ...
        (users_emb, pos_emb, neg_emb,
         userEmb0, posEmb0, negEmb0) \
            = self.getEmbedding(users.long(), keep_pos_items.long(),
                                need_replace_pos_item.long(),
                                replaceable_items_feature, neg.long())
